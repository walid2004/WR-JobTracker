...
def run_test():
    print("=" * 70)
    print("Testing Step 2 Matchmaker and Database State Machine")
    print("=" * 70)
    ...
    bmw_app_id = res1.application_id
    ...
    assert res2.application_id is not None
    assert res2.application_id != bmw_app_id
    ...
    assert res3.application_id == bmw_app_id

    print("\n" + "-" * 50)
    print("Verifying Persisted Cards and Timelines in SQLite:")
    apps = db.get_all_applications()
    for app in apps:
        details = db.get_application_by_id(app["id"])
        print(f"\nCard #{details['id']}: {details['company_name']} | Role: {details['job_title']} | Status: [{details['status']}]")
